...
####Note to Cory: If you want to move all relevant code for imputing/filter/etc data to here - above my analysis - so that it flows into cbMen/cbWomen. I used these two dataframes and combined them to AllFinishers because I'm lazy and hate base R.  Dplyr or die.  
...
AllFinishers %>% group_by(year) %>% summarise(RunnerCount=n())
...
AllFinishers %>%  ggplot(aes(x=as.factor(year), y=age, color=sex)) + geom_boxplot() + theme_light() + scale_color_manual(values=c("#4286f4", "#fc41e9")) + theme(legend.position = "none") + scale_y_continuous(labels=comma, "Age") +ggtitle("") + scale_x_discrete("Year") + ggtitle("Distribution of Age for Runners, 1999-2012") + facet_wrap(~sex, ncol=1)
```
...
PredictedTime <- predict(RunModelMen, newdata = ages)
...
FinishersWithPredicted %>% filter(year %in% c(1999, 2012)) %>% ggplot(aes(x=age, NormalizedTime, color=sex, group=sex)) + geom_point(alpha=0.15) + theme_light() + facet_grid(sex~year) + scale_color_manual(values=c("#4286f4", "#fc41e9")) + theme(legend.position = "bottom") + scale_y_continuous("Time Normalized to Predicted Fastest Time")
...
plot(density(100*time99Norm_m, na.rm = TRUE), 
    # ylim = c(0, 0.05), 
     col = "purple", ylim=c(0,0.05),
     lwd = 3,  xlab = "Time (percentage)",
     main = "Time Distribution for 1999 and 2012 Runners\n Percentage of the fastest runner for that age")
lines(density(100*time99Norm_w, na.rm = TRUE), 
          lwd = 3, col = "green")
legend("topleft", fill = c("purple", "green"),
       legend = c("1999M", "1999W"), bty = "n")


qqplot(time99Norm_m, time99Norm_w, pch = 19, cex = 0.5, 
       xlab = "Age Normalized Time in 1999 Race for Men",
       ylab = "Age Normalized Time in 1999 Race for Women", 
       main = "Quantile-quantile plot of female runner's performance")
abline(a =0, b = 1, col="red", lwd = 2)



plot(density(100*time12Norm_m, na.rm = TRUE), 
     ylim = c(0, 0.02), 
     col = "purple",
     lwd = 3,  xlab = "Time (percentage)",
     main = "Time Distribution for 2012 Runners Age Normalized Results \n Men v Women")
lines(density(100*time12Norm_w, na.rm = TRUE), 
          lwd = 3, col = "green")
legend("topleft", fill = c("purple", "green"),
...
qqplot(time12Norm_m, time12Norm_w, pch = 19, cex = 0.5, 
       xlab = "Age Normalized Time in 2012 Race for Men",
       ylab = "Age Normalized Time in 2012 Race for Women", 
       main = "Quantile-quantile plot of female runner's performance")
abline(a =0, b = 1, col="red", lwd = 2)

t.test(time12Norm_m, time12Norm_w)
