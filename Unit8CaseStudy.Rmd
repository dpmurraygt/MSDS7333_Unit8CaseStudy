...
This relationship between age and performance is a natural phenomenon known well by competitive runners. To account for this phenomenon, age grading is used for ranking runners performance based on their age. In fact, age graded performance is more frequently used for runner comparisons. Runnersworld.com describes age grading as "a way of putting all race participants on a level playing field, regardless of age or gender. (3)" In age grading, a runner's time is "graded" or normalized based on the world record time for his or her age. 

Given overwhelming visual evidence of the positive relationship between age and running performance, we implement an age grading method to compare the 1999 results to the 2012 race results for the Cherry Blossom Ten Mile Run. We use the fastest times recorded for each age considering the time span of 1999 to 2012. Many of these times are fairly close to world record times for 10 mile runs. Additionally, we apply a local regression (LOESS) model to predict the fastest times. LOESS is a non-parametric local regression implementation that uses a subset of data to fit individual regression models. This method allows for complex curve fitting. In our case, it takes some of the noise out of the data we use to normalize run times in order to create age grades.

We use the raw fastest times and ages to fit the LOESS model. By predicting the fastest times for the ages we are interested in, we are able to smooth the data. Using predicted times helps control for outliers and minimize year to year fluctuations in our dataset. We use these predicted fastest times to age grade each run time in 1999 and 2012, the two years we are interested in comparing. 
...
To compute age graded run times, we normalize raw run times in 1999 and 2012 by dividing by the fastest predicted time for all valid ages. Only times for ages greater than 11 and less than 81 are used. The Cherry Blossom Race requires all ten mile runners to be at least 12 years old and data for individuals older than 80 is extremely sparse. For example, a runner who is 33 with a finish time of 75 minutes would have a standardized time of (75/57) 1.32 given a predicted fastest time of 57 minutes for his age category.
