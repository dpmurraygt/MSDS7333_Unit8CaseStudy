...
Participation in competitive running has increased dramtically in the past two decades. Simultaneously, the rapid expansion of the Internet has resulted in massive quantities of data now freely available online. Given the appropriate tools, acquiring running data stored on a website is now a trivial task. In this paper, we analyze the performance of runners in the Cherry Blossom Ten Mile Run held annually in Washington, DC. Performance data is acquired by scraping the Cherryblossom.org race result archives for 1999 to 2012. We compare age-normalized race results for 1999 and 2012 men and women. Results show that 1999 runners performed better when using age grading. Further, men typically outperformed women in both years considered.

## Introduction

The sport of running continues to grow rapidly. While major events like the Boston Marathon have staged competitions for more than 100 years, the modern day sport of running has seen a significant expansion of runner participation since 1995. Overall participation in races around the United States expanded from 7 million in 1995 to almost 16 million in 2012 (5). 

The Cherry Blossom Ten Mile Race in Washington, D.C. represents a microcosm of the upward participation trend in running. Between the period of 1999 and 2012, participation has tripled. Runners of all ages and skill levels participate in the race each year. 
...
Race results for the 1999 to 2012 period are accessible in HTML tables at Cherryblossom.org. While we explore overall trends and runner characteristics, our objective is to compare the distribution of age normalized run times for 1999 and 2012. Addtionally, we compare gender times for each year.

It is of secondary interest to discover whether the increase in field size is of runners of all skill levels, or if the trend may signal expansion of the sport towards the general public and generally away from the elite sub-population of runners.
...
Modeling human running performance over time has been a classical topic in medical, sports and evolutionary journals.  These studies have been conducted in order to more accurately determine where the human ability for long distance running originated, how bodies predisposed to running, and the current impacts of diet and training in speed training. Ultimately, the data from the Cherry Blossom race shows the increasing popularity of long distance running, and the overall factors that age plays in modeling performance.
...
After downloading all the files available for both the men and women’s results from 1999 to 2012 we observed that the documents had different formats over the years. Before running our analysis, some major data cleanup needs to take place. The next few paragraphs will go over several format differences from one file to the next and how the issues were addressed using R. 

An easy way to read certain lines within a text file directly from the R command window is with the command “readLines”. One can store the whole file in a variable and then read specific lines that need to be examined. 
After inspecting all files, we can itemize the minimum requirements:
-	Each file has column headers for ranking, name, age, hometown, and time. 
-	Each column header is underlined by equal signs. 
-	Some of the files include extra lines at the top and bottom of the document. 
-	Some years include both the net time and the gun time, but we’ll only use the net time. 
-	For any missing entry, use N/A. 

We need to ensure that all the column headers are the same from year to year. For example, the column for age is labeled as “Ag” or “ag”. The R function “tolower” fixes this quickly by changing all column headers to lower case letters.

The width of the columns might be different from file to file, so our code needs to be generalized to all files. In the “equal signs” row (the underline for the headers), we’ll use the position of the “blanks” to find the exact location of the columns for each file and store these numerical values in an array. 

We also observed that the 2003 Men’s file has an added blank space in front of the numeric value for age. Basically, the age column does not align correctly with the “equal signs” row for the age variable. Since the location of each variable is stored in an array, we can easily shift the index by one to resolve this issue. This fix also applies to the 2006 Men’s file which had a similar issue. 

Some entries show runners that finished the race with an age of zero. Clearly the ages were missing and the “0’s” were added instead of “N/A’s” so these can be changed without difficulty. 

As for the runtime, the variable is given in hours, minutes, and seconds. For simplicity we transformed it to minutes only. There were a few entries we had to remove as some runners did not finish the race and their corresponding times were given as “#”, “*”, or a blank space. 

After all these edits were made, the files were read and the data was stored in a data frame with the following variables: year, sex, name, age, hometown, and runtime. 
...
AllFinishers %>% 
  group_by(year, sex) %>% 
    summarise(RunnerCount = n()) %>% 
      ...
      scale_y_continuous( "Total Runners", labels = comma) +ggtitle("Cherry Blossom 10 Mile Race - Participants by Gender, 1999-2012") 

AllFinishers %>% 
  group_by(year) %>% 
    summarise(RunnerCount=n())
```
From 1999 to 2012 the overall size of the race grew, and in fact more than doubled in size.  While participation by men and women both grew substantially, participation by women played the greatest role in the expansion of the field.  Women's participation comprised a minority, 42% of runners, but by 2012 had exchanged positions with men at 57.5% of all participants.  The two genders were at parity levels of participation in 2005, and women have since added more than 5,000 participants.
...
The age of male runners has declined since 1999, with an average age of 40.3 in 1999, declining to a minimum of 37.8 in 2008, and stabilizing between 37 and 38 years of age since 2008.  The standard deviation of mens' ages has consistently been between 10 and 12 years.
...
BestTimeYr %>% filter(age %in% (20:80)) %>%
...
#### Modeling Mens' Finishing Times By Age

Employing the LOESS regression method discussed in the methods section, we will create a model to predict the time of the first finishing male for each age between 20 years old and 80 years old.  The exclusion of younger and older ages is reflective of the lower participation rates, and higher level of variability in these ages finishing times.    

``` {r LOESS_analysis_setup fig.width=8.5, fig.height=4}
...
BestTimesM %>% ggplot(aes(x=age, y=fastest, group=sex, color=sex)) + geom_line() + theme_light() + ggtitle("Fastest Male Race Time Across 1999-2012") + scale_x_continuous("Age") + scale_y_continuous("Run Time in Minutes") + geom_smooth(method = "loess") + scale_color_manual(values = c("#4286f4")) + theme(legend.position = "none")
...
```
The LOESS regression results in a curve, that similar to what was shown in our exploration, predicting the best run time in a runner's late 20's, with a degrading performance annually through the rest of the runner's career.  The degradation of performance becomes more severe after age 68.  This model would also suggest that the best runners at age 20 and age 60 are equal in performance in the ten mile distance of the Cherry Blossom course.
...
```
Reviewing the density plot, the 1999 field is shown to be centered just above 1.6 times the fastest runner's time for the respective age group.  2012 is centered closer to 1.75 times the fastest runner's time.  2012 also exhibits a wider, flatter curve with a less prominent median normalized run time.
...
```
?Yeah not really sure what to say about QQ?
...
We will also model, and compare Womens' finishing times between 1999 and 2012 using the same Loess Regression technique to build a model to predict the fastest woman finisher's time for each age between 20 years old and 80 years old.
