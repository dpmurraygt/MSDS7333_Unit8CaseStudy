...
AllNormTimes %>% 
          ...
          theme_light() + 
          theme(legend.position = "bottom") + 
          ggtitle("Figure XX: Density of Normalized Run Time - Women, 1999 vs. 2012", 
                    subtitle = "Run Times Normalized by Fastest Predicted Time (LOESS) for All Race Years")
...
ggplot(df) + 
          ...
          geom_abline(aes(intercept=0, slope=1, col="red"), show.legend = FALSE)+
          theme_light() + 
          ggtitle("Figure XX: QQ Plot of Age Graded Times for Women") + 
          scale_x_continuous("Age Normalized Time - 1999") + 
          scale_y_continuous("Age Normalized Time - 2012") + 
          theme(legend.position="bottom", 
                legend.title=element_blank())
...
Comparing the results of 1999 and 2012, women's times reveal a less evident change than we saw for the men. However, 2012 results are very similar to those of the men, with a wider distribution and flatter center of age graded times. The median run time increases from 1.73 to 1.78, and the mean from 1.72 to 1.80.  The interquartile range has also expanded from 0.33 to 0.38.  

As seen in the previous section, the increase in female particpants expanded dramatically. The distributions of women's normalized run times for 1999 and 2012 is more similar than the distributions for men.  While additional results have been added to the tail of the distrbution, the overall length of the distribution has not changed in the same manner that the men's times has.  The maximum race length is still about three times the first finisher for each age group.
...
A two-sample Student's t-test finds a difference between the two years of women's race results after age grading.  While there was a similar normalized median, the difference of normalized mean running times is approximately 0.07 (p<0.0001).  
...
```
In 1999, we can infer that the performance of the men's field is more closely aligned to the elite portion of their field than the women's field is to their elite competitors.  The median male performance is 1.6x the time of the first place finisher for their age, while females are above 1.75.  
...
Reviewing the density curves of 2012, the median male performance relative to fastest age performance has shifted to be very similar to the womens.  The distribution of the women is still a more narrow interquartile range than the men.   
