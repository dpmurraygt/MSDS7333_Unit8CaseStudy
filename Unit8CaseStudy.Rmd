...
BestTimeYr %>% 
  filter(age %in% (20:80)) %>%
    ...
    theme(legend.position = "bottom") + 
    ggtitle("Figure 5: Fastest Time by Age by Gender, 1999-2012") + 
    scale_x_continuous(limits=c(20,80)) + 
    scale_y_continuous("Best Performance by Age, 1999-2012")
```

Given visual evidence of the positive relationship between age and running performance, we implement an age grading method to compare the 1999 race results to the 2012 race results. Age grading allows us to objectively determine differences between two race results by controlling for the effect age has on running. Additionally, in order to smooth out the fastest times recorded for ages with smaller numbers of participants, we apply a local regression (LOESS) method to predict fastest times. This allows for a more consistent analysis and helps to control outliers given our dataset is small in comparison to the population of times for all races around the world. This smoothing can be seen for men in Figure 6.
...
``` {r men_1999to2012, fig.width=8.5, fig.height=4}
...
BestTimesM %>% 
  ...
  theme_light() + 
  ggtitle("Figure 6: Fastest Male Race Times - 1999 to 2012") + 
...
We standardize run times by dividing by the fastest predicted time for all valid ages competing in the Cherry Blossom Ten Mile Run. In order to obtain the fastest times, we consider the entire time span of 1999 to 2012. Further, only times for ages greater than 11 and less than 81 are used. The Cherry Blossom Race requires runners to be at least 12 years old and data for individuals older than 80 is extremely sparse. As an example of standardization, a runner who is 33 with a finish time of 75 minutes in the 10 mile run would have a standardized time of (75/57) 1.32 given a predicted fastest time of 57 minutes for the age 33 category.
...
The LOESS regression results in a curve similar to what was shown in our previous data exploration. predicting the best run time in a runner's late 20's, with a degrading performance annually through the rest of the runner's career.  The degradation of performance becomes more severe after age 68.  This model would also suggest that the best runners at age 15 and age 60 are equal in performance in the ten mile distance of the Cherry Blossom course. However, this is likely to be an anomaly given much fewer runners in the 15 and under age group.

```{r normalize_men, fig.width=8.5, fig.height=4, eval=FALSE}
