...
####Note to Dennis: Added in imputation. Also added in normalized analysis. Should be ready
#### to write up. Salient points: kept all data in for predictions after imputation, reduced span
#### to 0.5 to catch curves better but not too much. Adjusted some of your code but viz works
#### there are a couple viz in EDA sections that call for an object you likely had in memory
#### but is not in .RMD
```{r imputation, include=FALSE, echo=FALSE}
...
# could cut VAR on country, obvious indicator for runTime and age
AllFinishersImp <- AllFinishers[,c("age","runTime", "year","sex")]

# this doesnt matter unless we do agg analysis including women
# call binary indicator to speed up imputation process a bit
AllFinishersImp <- AllFinishersImp %>% 
...
set.seed(42)
ImputeObject <- missForest(AllFinishersImp, 
                            ntree = 50, 
                            maxiter = 3, # set back to 3
                            variablewise = FALSE,
                            verbose = FALSE)

# all numeric, small normalized RMSE
ImputeObject$OOBerror 

# get imputed features and round age predictions
AllFinishersImp <- ImputeObject$ximp
AllFinishersImp$age <- round(AllFinishersImp$age)

# column bind other features not included in impute
# to re-create full data set
AllFinishers <- cbind(AllFinishers[, c("sex","name","home")],
                      AllFinishersImp[,-4] )
any(is.na(AllFinishers$age))

# what does imputation look like for age?
AllFinishersImp[which(is.na(AllFinishers$age)),]
