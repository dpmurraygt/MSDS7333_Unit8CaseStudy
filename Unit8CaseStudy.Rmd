...
dir <- "C:/Users/Julien/Documents/Git/MSDS7333_Unit08"
setwd(dir)
...
Nowadays, thanks to the rapid expansion of the internet, there is a massive quantity of free data available online. Scraping and analyzing this data can bring some insights into a problem or bring a different perspective to solve that problem. An example of easily obtainable data is the Cherry Blossom Ten Mile run. Started in 1973, the race is held in Washington DC. Every year, the organizers publish the results online. In this paper we first discuss how to scrape and clean up the data for the years 1999 to 2012. We then run a statistical analysis and investigate the times of the runners over the years for both men and women. The results show that …
...
# replace ages < 5, 10M is for 13 and up, likely errors
cbMen <- cbMen %>% 
          mutate(age, age=replace(age, age < 13, NA))

# which did we null out?
nulls <- which(is.na(cbMen$age))

# clean up whitespace just because it's obnoxious
cbMen[,c("sex","name","home")] <- apply(cbMen[,c("sex", "name", "home")], 
                                        2, 
                                        trimws)
...
cbMen_imp <- cbMen[c("age","runTime", "year","sex")]

# this doesnt matter unless we do agg analysis including women
cbMen_imp <- cbMen_short %>% 
                  mutate(sex = ifelse(sex == "M", 1, 0 ))


# RF (non parametric) impute
imp_object <- missForest(cbMen_imp)

# all numeric, small normalized RMSE
imp_object$OOBerror 

cbMen_imp <- imp_object$ximp
cbMen_imp$age <- round(cbMen_imp$age)

# imputed cbMen
cbMen_imp <- cbind(cbMen[, c("sex","name","home")],
                   cbMen_imp[,-4] )


# this is our assignment - age grading
fastestMen = tapply(cbMen_imp$runTime, 
                    cbMen_imp$age,
                    min, 
                    na.rm = FALSE) # we imputed here instead

# one outlier at 1.5 - Peter Hui, had different format, fix
fastestMen[61] = 66.53
plot(fastestMen ~ names(fastestMen), type ="l", xlim = c(10, 80))
# convert to numeric
age_fastest_men = as.numeric(names(fastestMen))
# smooth fastest times by fitting and predicting LOESS
mens_fast_lfit = loess(fastestMen ~ age_fastest_men)
