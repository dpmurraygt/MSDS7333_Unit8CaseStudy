...
```{r}
#condense this down to a data frame
#we don't need a list of data frames, just give me a single data frame for everything and then we can filter as needed



MenAll <- plyr::ldply(menDF, data.frame)
WomenAll <- plyr::ldply(womenDF, data.frame)

AllFinishers <- rbind(MenAll, WomenAll)
...
AllFinishers <- AllFinishers %>% group_by(year)  %>% mutate(OverallPlace = rank(runTime, ties.method="first")) %>%
  group_by(year, sex) %>% mutate(GenderPlace = rank(runTime, ties.method="first")) %>% data.frame()
...
#then remove it, and rerank everything

AllFinishers <- AllFinishers %>% filter(runTime > Threshold) %>% group_by(year)  %>% mutate(OverallPlace = rank(runTime, ties.method="first")) %>%
  group_by(year, sex) %>% mutate(GenderPlace = rank(runTime, ties.method="first")) %>% data.frame()
...
```{r}
#add in the age groups used for the race
...
```{r}
#viz it up
#no base graphics here, make it look good in ggplot with good viz principles
...
AllFinishers %>% filter(GenderPlace==1) %>% mutate(year=as.factor(year)) %>%
  ggplot(aes(x=year, y=runTime, group=sex, color=sex)) + geom_point() + theme_light() + scale_color_manual(values=c("#4286f4", "#fc41e9")) + theme(legend.position = "bottom")
```

```{r fig.width=8.5, fig.height=4}

AllFinishers %>% filter(GenderPlace==1) %>% mutate(year=as.factor(year)) %>%
  ggplot(aes(x=year, y=runTime, group=sex, color=sex)) + geom_point() + theme_light() + scale_color_manual(values=c("#4286f4", "#fc41e9")) + facet_grid(~AgeGroup) + theme(legend.position="bottom")
